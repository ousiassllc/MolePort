...
ボーダーの占有サイズ（上下 2 行、左右 2 列）を各パネルの高さ・幅計算に反映する:
...
| コンポーネント | ファイル | 変更内容 |
|--------------|---------|---------|
| **DashboardPage** | `pages/dashboard.go` | `atoms.RenderDivider` 呼び出しを削除。各パネルの View を `FocusedBorder` / `UnfocusedBorder` で囲む |
| **ForwardPanel** | `organisms/forwardpanel.go` | セクションタイトルをボーダーのインラインタイトルに移動。`FocusIndicator` (`▌`) を廃止 |
| **SetupPanel** | `organisms/setuppanel_view.go` | 同上 |
| **LogPanel** | `organisms/logpanel.go` | タイトル「Log」をボーダーのインラインタイトルとして追加 |
| **StatusBar** | `organisms/statusbar.go` | `StatusBarStyle`（背景色付き）を適用 |
| **ConfirmDialog** | `molecules/confirmdialog.go` | `FocusedBorder` でダイアログを囲む |
| **PasswordInput** | `molecules/passwordinput.go` | `FocusedBorder` でダイアログを囲む |

#### ボーダーのインラインタイトル

Lip Gloss v1.x の `BorderTitle` 機能を利用して、パネルタイトルをボーダー上辺に表示する:

```go
// 例: ForwardPanel のボーダー描画
...
| 2.6 | 2026-02-28 | TUI ビジュアル改善セクション追加: スタイル変更、レイアウト計算変更、各コンポーネント View 変更の詳細を記載 | #29 TUI ビジュアル改善 |
