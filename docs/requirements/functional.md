...
バイナリ名は `moleport` を正式名とし、`mp` をエイリアスとして提供する（インストール時に両方を配置）。
以下の仕様では `moleport` で記載するが、`mp` でも同様に動作する。
...
| F-07 | CLI サブコマンド | サブコマンド形式での全操作（daemon/connect/add/delete/start/stop/list/status/tui） | 必須 |
...
| F-18 | コマンドエイリアス | `mp` を `moleport` のエイリアスとして提供 | 必須 |
...
mp <subcommand> [options] [arguments]     # エイリアス
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│  MolePort v0.2.0                            ⬤ Daemon Connected  │
├─────────────────────────────────────────────────────────────────┤
│  SSH Hosts                                                      │
│  ┌───────────────────────────────────────────────────────────┐  │
...
| 2.0 | 2026-02-11 | デーモン化対応: CLI サブコマンド体系追加、ユースケースをデーモン/クライアント構成に改訂、mp エイリアス追加 | デーモン化対応 |
