...
- **主要コンポーネント**:
  ...
  - `Resolver`: 言語の解決（config.yaml → 環境変数 → デフォルト）
...
- **CLI**: `main.go` でコマンド実行前に `i18n.Resolver` で言語を解決し、`i18n.SetLang()` を呼び出す
...
│   │   │   ├── handler_forward.go     # forward.add/delete/start/stop/list
...
│   │       ├── langpage.go            # LangPage（言語選択画面）
│   │       └── themepage.go           # ThemePage（テーマ選択画面）
...
graph TD
    subgraph Daemon["デーモンプロセス"]
        ...
        Reconnect --> |成功| Restore["Forward 復元"]
        Metrics --> |イベント| Broker
        Broker --> |通知| Client2
    end
```
